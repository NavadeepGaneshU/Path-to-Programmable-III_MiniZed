...
void NavDemo_Run(void) {
   ...
   alt = NavDemo_ConvFeetToMeters(3116.8); // altitude for Bengaluru, KA in meters
   ...
   usleep(100000);
      ...
      //printf("Pmod NAV: %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.1f, %s\n\r",nav.acclData.X,nav.acclData.Y,nav.acclData.Z,nav.gyroData.X,nav.gyroData.Y,nav.gyroData.Z,nav.hPa,str);
      printf("%.2f %.2f %.2f\n",nav.gyroData.X,nav.gyroData.Y,nav.gyroData.Z);	//basic flight sim processing ide

      /* print this on the TFT display */
      GUI_DisString_EN(50,50,"Compass",&Font12,GUI_BACKGROUND,CYAN);
      sprintf(compassbuf, "%s", str);
      GUI_DisString_EN(50,65,compassbuf,&Font12,GUI_BACKGROUND,YELLOW);
    }
