...
* @file interrupt_hw.c
*
* This file contains a design example for testing hardware interrupts on this platform
*
* The example uses the interrupt capability of the GPIO to detect push button
* events and set the output LEDs based on the input and also hardware interrupt based on edge detection
*
* @note
...
int main(void)
{
	xil_printf("/**********GIC Hardware Interrupt Example Test**********/\r\n");
...
void GpioIntrExample(XScuGic *Intc, XGpioPs *Gpio, u16 DeviceId, u16 GpioIntrId)
{
	...
	xil_printf("Press the button to interrupt\n\r");
...
* the GPIO device. This service routine turns the RED LED ON for 1 second
*
...
static int SetupInterruptSystem(XScuGic *GicInstancePtr, XGpioPs *Gpio,u16 GpioIntrId)
{
	...
	XGpioPs_SetIntrType(Gpio, GPIO_BANK, 0x01, 0xFFFFFFFF, XGPIOPS_IRQ_TYPE_EDGE_RISING);
